...
public class Model {
    private Box[][] myField = new Box[Picture.COLUMNS][Picture.ROWS]; //матрица с боксами игрового поля 1го игрока
    private Box[][] enemyField = new Box[Picture.COLUMNS][Picture.ROWS];  //матрица с боксами игрового поля соперника
    private List<Ship> shipsOneDeck = new ArrayList<>(); //список всех однопалубных кораблей 1го игрока
    private List<Ship> shipsTwoDeck = new ArrayList<>(); //список всех двухпалубных кораблей 1го игрока
    private List<Ship> shipsThreeDeck = new ArrayList<>(); //список всех трехпалубных кораблей 1го игрока
    private List<Ship> shipsFourDeck = new ArrayList<>(); //список всех четырехпалубных кораблей 1го игрока
    private List<Ship> allShipsOfEnemy = new ArrayList<>(); //список всех кораблей соперника
    ...
    //метод, который возвращает список всех кораблей 1го игрока
    ...
    //метод, который возвращает все боксы из всех кораблей 1го игрока
    ...
    //метод, устанавливающий значение указанного бокса в указанную матрицу (игрового поля)
    ...
    //метод, возвращающий бокс из указанной матрицы (игрового поля) по координатам панели отрисовки игрового поля
    ...
    //метод, который устанавливает значение isOpen в true (открывает боксы после попадания в корабль) боксов,
    //находящихся рядом с боксом корабля, определенного входными координатами
    ...
    //открывает все пустые боксы по периметру корабля, используется в случае, когда все палубы корабля подбиты
    public void openAllBoxesAroundShip(Ship ship) {
        //циклами открываются все боксы вокруг первого и последнего боксов корабля, т.к. больше 4 палуб быть не может, то
    ...
    //удаляет корабль из соответствующего списка
    // используется в процессе расстановки и удалении кораблей на своем игровом поле
