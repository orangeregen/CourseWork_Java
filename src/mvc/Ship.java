...
public class Ship implements Serializable {
    ...
    private List<Box> boxesOfShip; //список всех боксов данного корабля
    private final boolean isHorizontalPlacement; //ориентация корабля

    //возвращает список всех боксов данного корабля
    ...
    //возвращает количество палуб корабля
    public int getCountDeck() {
        return countDeck;
    }

    //для конкретного корабля получаем информацию об
    //ориентации, палубности и о боксах
    ...
    //метод создания корабля горизонтальной ориентации по заданным координатам x и y
    public void createHorizontalShip(int x, int y) {
        //для того чтобы корабль не отрисовался за пределами игрового поля,
        //высчитываем максимально возможную координату по X для отрисовки с учетом количества палуб
        int pointLimitValueForPaint = (Picture.COLUMNS - countDeck) * Picture.IMAGE_SIZE;
        for (int i = 0; i < countDeck; i++) {
            Box newBox;
            //если Х больше максимально допустимой точки отрисовки,
            // то координата по Х для бокса = pointLimitValueForPaint + i * Picture.IMAGE_SIZE
            if (x > pointLimitValueForPaint) {
                //крайняя точка отрисовки + количество палуб * 40 (имедж сайз)
